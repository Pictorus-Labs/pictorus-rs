...
mod tests {
        ...
        ($type:ty) => {
            paste! {
                ...
                fn [<test_right_shift_scalar_ $type>]() {
                    ...
                    let output = block.process(&params, &model_clock, [<8 $type>]);
                    assert_eq!(output, [<2 $type>]);
                    assert_eq!(block.buffer(), [<2 $type>]);

                    let output = block.process(&params, &model_clock, [<2 $type>]);
    ...
    fn shift_amount_exceeds_width_panics() {
        ...
        let output = block.process(&params, &model_clock, 1u8);
        assert_eq!(output, 0);
    }
    ...
    fn float_shift_amount_exceeds_cast_width_panics() {
        ...
        let output = block.process(&params, &model_clock, 1.0f32);
